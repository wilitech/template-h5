...
iSlider.prototype = {
  ...
  init: function() {
    var self = this
    this.wrap =
      typeof this.opts.wrap == 'string'
        ? this.$(this.opts.wrap)
        : this.opts.wrap

    if (this.SS) {
      //使用sessionStorage来保存当前浏览到第几页了   后退回来的时候 定位到这一页
      this._sessionKey = btoa(
        encodeURIComponent(
          this._sessionKey + this.wrap.id + this.wrap.className
        )
      )

      var lastLocateIndex = parseInt(this.SS[this._sessionKey])
      this.index =
        this.opts.lastLocate && lastLocateIndex >= 0 ? lastLocateIndex : 0
    } else {
    ...
    this._tpl = this.opts.item
      ? this._tpl.querySelectorAll(this.opts.item)
      : this._tpl.children

    for (var i = 0; i < this._tpl.length; i++) {
      this._tpl[i].style.cssText +=
  ...
  _setHTML: function(index) {
    ...
    if (this.index > 0) {
      ...
      this._prev.style.cssText += this._getTransform(
        '-' + this.scrollDist + 'px'
      )
  ...
  _touchstart: function(e) {
    ...
    this.touchInitPos = this.opts.isVertical
      ? e.touches[0].pageY
      : e.touches[0].pageX // 每次move的触点位置
  ...
  _touchmove: function(e) {
    ...
    if (this.opts.noslide && this.opts.noslide.indexOf(this.index) >= 0) {
      ...
      if (
        !this.opts.noslideBack ||
        (this.opts.isVertical
          ? e.touches[0].pageY - this._touchstartY < 0
          : e.touches[0].pageX - this._touchstartX < 0)
      ) {
        return
      }
    }

    var currentX = this.opts.isVertical
      ? e.touches[0].pageY
      : e.touches[0].pageX
  ...
  _itemReset: function() {
    var self = this
    self._current.style.cssText +=
      '-webkit-transition-duration:' +
      this.opts.speed +
      'ms;' +
      this._getTransform(0)
    if (self._prev) {
      self._prev.style.cssText +=
        '-webkit-transition-duration:' +
        this.opts.speed +
        'ms;' +
        this._getTransform('-' + this.scrollDist + 'px')
    }
    if (self._next) {
      self._next.style.cssText +=
        '-webkit-transition-duration:' +
        this.opts.speed +
        'ms;' +
        this._getTransform(this.scrollDist + 'px')
  ...
  prev: function() {
    ...
    setTimeout(function() {
      if (self.$('.' + self.opts.playClass, self.wrap)) {
        self.removeClass(
          self.$('.' + self.opts.playClass, self.wrap),
          self.opts.playClass
        )
      }
      self.addClass(self._current, self.opts.playClass)
      ...
      self._prev.style.cssText +=
        ...
        self._getTransform('-' + self.scrollDist + 'px')
